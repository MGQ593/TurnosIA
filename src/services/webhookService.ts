...
interface TurnoFinalizadoWebhookData {
  id_turno: number; // ID único del turno
  numero_turno: string; // Número de turno visible
  agencia_id: number; // ID de la agencia
  modulo: string; // Módulo donde se atendió
  asesor: string; // Asesor que atendió
  estado: string; // Estado del turno (finalizado)
...
interface TurnoCanceladoWebhookData {
  id_turno: number; // ID único del turno
  numero_turno: string; // Número de turno visible
  agencia_id: number; // ID de la agencia
  modulo?: string; // Módulo asignado (si fue llamado)
  asesor?: string; // Asesor asignado (si fue llamado)
  estado: string; // Estado del turno (cancelado)
...
/**
 * Envía los datos de finalización de turno al webhook de n8n
 * @param data Datos de la finalización del turno
 * @param maxRetries Número máximo de reintentos (default: 3)
 * @returns Resultado del envío con información de intentos
 */
export async function enviarFinalizacionTurnoWebhook(
  data: TurnoFinalizadoWebhookData,
  maxRetries: number = 3
): Promise<WebhookResult> {
  const webhookUrl = process.env.WEBHOOK_TURNOS_URL;
  ...
  const payload = {
    tipo_evento: 'turno_finalizado', // ⭐ Identificador del tipo de evento
    id_turno: data.id_turno,
    numero_turno: data.numero_turno,
    agencia_id: data.agencia_id,
    modulo: data.modulo,
    ...
    tiempo_atencion_minutos: data.tiempo_atencion_minutos,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`📤 Intento ${attempt}/${maxRetries} - Enviando finalización de turno ${data.numero_turno} al webhook n8n`);
      console.log(`📱 Datos del webhook (finalización):`, JSON.stringify(payload, null, 2));
      ...
      console.log(`✅ Finalización de turno ${data.numero_turno} enviada exitosamente al webhook en intento ${attempt}`);

      return {
        success: true,
        message: 'Notificación de finalización enviada correctamente',
        attempts: attempt
      };
  ...
  // Si llegamos aquí, todos los intentos fallaron
  console.error(`❌ Webhook de finalización falló después de ${maxRetries} intentos:`, lastError?.message);

  return {
    success: false,
    message: `Error al enviar notificación de finalización después de ${maxRetries} intentos: ${lastError?.message}`,
    attempts: maxRetries
  };
}

/**
 * Envía los datos de cancelación de turno al webhook de n8n
 * @param data Datos de la cancelación del turno
 * @param maxRetries Número máximo de reintentos (default: 3)
 * @returns Resultado del envío con información de intentos
 */
export async function enviarCancelacionTurnoWebhook(
  data: TurnoCanceladoWebhookData,
  maxRetries: number = 3
): Promise<WebhookResult> {
  const webhookUrl = process.env.WEBHOOK_TURNOS_URL;
  ...
  const payload = {
    tipo_evento: 'turno_cancelado', // ⭐ Identificador del tipo de evento
    id_turno: data.id_turno,
    numero_turno: data.numero_turno,
    agencia_id: data.agencia_id,
    modulo: data.modulo,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`📤 Intento ${attempt}/${maxRetries} - Enviando cancelación de turno ${data.numero_turno} al webhook n8n`);
      console.log(`📱 Datos del webhook (cancelación):`, JSON.stringify(payload, null, 2));
      ...
      console.log(`✅ Cancelación de turno ${data.numero_turno} enviada exitosamente al webhook en intento ${attempt}`);
  ...
  // Si llegamos aquí, todos los intentos fallaron
  console.error(`❌ Webhook de cancelación falló después de ${maxRetries} intentos:`, lastError?.message);

  return {
    success: false,
    message: `Error al enviar notificación de cancelación después de ${maxRetries} intentos: ${lastError?.message}`,
    attempts: maxRetries
  };
}
